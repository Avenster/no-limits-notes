...
export default function LoginPage() {
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center bg-[#0c0c0d] px-6">
      {/* faint paper grain, no color wash */}
      <div
        aria-hidden
      ...
      <div className="absolute inset-x-0 top-0 z-10 flex items-center justify-between px-6 py-6">
        <button
          ...
          onClick={() => navigate(-1)}
          aria-label="Go back"
          className="group flex items-center gap-2 rounded-full border border-white/[0.08] bg-white/[0.02] px-3.5 py-2 text-sm text-neutral-500 backdrop-blur-xl transition-colors duration-200 hover:border-white/[0.14] hover:text-neutral-300"
        >
      ...
      <div className="relative z-10 w-full max-w-sm animate-[fadeIn_0.6s_ease-out]">
        <div className="rounded-[20px] border border-white/[0.08] bg-[#111113] px-8 py-10 shadow-[0_1px_0_0_rgba(255,255,255,0.04)_inset,0_24px_60px_-24px_rgba(0,0,0,0.6)]">
          <div className="mb-9 flex flex-col items-center gap-5 text-center">
            <NoteblockMark />

            <div className="flex flex-col gap-2">
              <h1 className="font-serif text-[26px] font-normal leading-tight tracking-tight text-neutral-50">
          ...
          <div className="flex flex-col gap-2.5">
            <OutlineLink href={`${backendUrl}/auth/google`}>
            ...
            <OutlineLink href={`${backendUrl}/auth/github`}>
              <span className="flex h-5 w-5 shrink-0 items-center justify-center">
                <GitHubGlyph />
              </span>
            ...
            <div className="my-1 flex items-center gap-3 px-1">
              ...
              <span className="text-[11px] uppercase tracking-[0.08em] text-neutral-600">or</span>
              <div className="h-px flex-1 bg-white/[0.07]" />
            </div>

            <OutlineLink href="/join" muted>
...
/**
 * The Noteblock mark: a quiet 2x2 grid, one cell dimmed — echoes the
 * app's logo without reproducing it as a flat icon-in-a-box.
 */
function NoteblockMark() {
  return (
    <div className="flex h-12 w-12 items-center justify-center rounded-[14px] border border-white/[0.08] bg-white/[0.02]">
      <div className="grid grid-cols-2 gap-[3px]">
        <div className="h-[9px] w-[9px] rounded-[2.5px] bg-neutral-100" />
        <div className="h-[9px] w-[9px] rounded-[2.5px] bg-neutral-500" />
        <div className="h-[9px] w-[9px] rounded-[2.5px] bg-neutral-600" />
        <div className="h-[9px] w-[9px] rounded-[2.5px] bg-neutral-100" />
      </div>
    </div>
  );
...
/**
 * A single flat, outlined control. Background stays close to the card
 * tone; only the border and text brighten slightly on hover — no scale,
 * no translate, no gradient fill.
 */
function OutlineLink({
  ...
  return (
    <a
      href={href}
      className={`flex items-center justify-center gap-3.5 rounded-xl border px-4 py-3.5 text-[14.5px] font-medium transition-colors duration-150 ${
        muted
          ? "border-white/[0.06] text-neutral-500 hover:border-white/[0.12] hover:text-neutral-300"
          : "border-white/[0.10] text-neutral-200 hover:border-white/[0.20] hover:text-white hover:bg-white/[0.02]"
      }`}
    >
      {children}
    </a>
...
function GitHubGlyph() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="#c9c9c9">
